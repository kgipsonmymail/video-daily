...
Source: https://platform.minimaxi.com/docs/api-reference/text-post
    ...
    ChatCompletionReq:
      ...
      properties:
        model:
          type: string
          description: >-
            模型 ID。可选值：`MiniMax-M2.7`, `MiniMax-M2.7-highspeed`, `MiniMax-M2.5`,
            `MiniMax-M2.5-highspeed`, `MiniMax-M2.1`, `MiniMax-M2`
            。注：`MiniMax-M2.7`, `MiniMax-M2.7-highspeed`, `MiniMax-M2.5`,
            `MiniMax-M2.5-highspeed`, `MiniMax-M2.1` 和 `MiniMax-M2`
            为推理模型，为获得最佳体验建议使用流式输出
          enum:
        ...
        tools:
          type: array
          description: 可供模型选择调用的工具列表。更多详情参考 [工具使用 & 交错思维链](/guides/text-m2-function-call)
          items:
            type: object
            required:
              - type
