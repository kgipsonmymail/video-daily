...
**目的：** 测试 MiniMax image-01 的人物一致性能力

**流程：**
1. 创建测试 → 定义角色（名称+英文描述）和变体类型
2. 生成基础图 → 每个角色生成一张基础图（t2i）
3. 生成变体图 → 使用 subject_reference 保持一致性，生成不同表情/动作/装备/形象
4. 评分 → 用户在前端表格中对每个变体进行 1-5 星评分

**技术实现：**
- 使用 `subject_reference: [{"type": "character", "image_file": "参考图URL"}]` 参数
- 参考图通过 `/files/{path}` 端点提供公网访问
- 后台异步生成，前端轮询状态

**前端页面：** `/consistency`
- 表格视图：行=角色，列=变体类型
- 每个单元格显示变体图 + 星级评分 + 备注输入
- 支持批量生成和单独评分
